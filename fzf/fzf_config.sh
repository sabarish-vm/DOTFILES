...
_fzf_comprun() {
    ...
    cd | z | mkdir)
        fd -t d -d 4 '.' "$dir" | fzf --prompt 'D>L4' \
            --bind "ctrl-h:transform:[[ ! \$FZF_PROMPT =~ H ]] &&
        echo 'change-prompt(HD>L4)+reload(fd -t d -d 4 --hidden . $dir)' ||
        echo 'change-prompt(D>L4)+reload(fd -t d  -d 4 . $dir)'" \
            --bind "ctrl-i:transform:[[ ! \$FZF_PROMPT =~ U ]] &&
        echo 'change-prompt(UD>L4)+reload(fd -t d -d 4  --unrestricted . $dir)' ||
        echo 'change-prompt(D>L4)+reload(fd -t d -d 4  . $dir)'" \
    ...
    *)
        fd -t f -d 4 '.' "$dir" | fzf -m --prompt 'F>L4 ' \
            --bind "ctrl-f:transform:[[ ! \$FZF_PROMPT =~ F ]] &&
            echo 'change-prompt(F>L4 )+reload(fd --type file -d 4  . $dir)' ||
            echo 'change-prompt(D>L4 )+reload(fd --type directory -d 4  . $dir)'" \
            --bind "ctrl-h:transform:[[ ! \$FZF_PROMPT =~ HF ]] && 
            echo 'change-prompt(HF>L4 )+reload(fd --type file -d 4  --hidden . $dir)' ||
            echo 'change-prompt(HD>L4 )+reload(fd --type directory -d 4  --hidden . $dir)'" \
            --bind "ctrl-o:transform:[[ ! \$FZF_PROMPT =~ UF ]] && 
            echo 'change-prompt(UF>L4 )+reload(fd --type file -d 4  --unrestricted . $dir)' ||
            echo 'change-prompt(UD>L4 )+reload(fd --type directory -d 4  --unrestricted . $dir)'" \
            --bind "shift-up:transform:$HOME/.dotfiles/fzf/fzf_level \$FZF_PROMPT up $dir" \
            --bind "shift-down:transform:$HOME/.dotfiles/fzf/fzf_level \$FZF_PROMPT down $dir" \
